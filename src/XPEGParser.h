#import <PEGKit/PKParser.h>

enum {
    XPEG_TOKEN_KIND_CLOSE_CURLY = 14,
    XPEG_TOKEN_KIND_TEXT,
    XPEG_TOKEN_KIND_GT_SYM,
    XPEG_TOKEN_KIND_ABBREVIATEDAXIS,
    XPEG_TOKEN_KIND_VALGT,
    XPEG_TOKEN_KIND_SWITCH,
    XPEG_TOKEN_KIND_LET,
    XPEG_TOKEN_KIND_FOLLOWING,
    XPEG_TOKEN_KIND_NAMESPACE,
    XPEG_TOKEN_KIND_RETURN,
    XPEG_TOKEN_KIND_IF,
    XPEG_TOKEN_KIND_INTERSECT,
    XPEG_TOKEN_KIND_TRUE,
    XPEG_TOKEN_KIND_ATTR,
    XPEG_TOKEN_KIND_VARIABLE,
    XPEG_TOKEN_KIND_SATISFIES,
    XPEG_TOKEN_KIND_SHIFT_LEFT,
    XPEG_TOKEN_KIND_VALLE,
    XPEG_TOKEN_KIND_LE_SYM,
    XPEG_TOKEN_KIND_FUNCTION,
    XPEG_TOKEN_KIND_COMMENT,
    XPEG_TOKEN_KIND_NODE,
    XPEG_TOKEN_KIND_IN,
    XPEG_TOKEN_KIND_TO,
    XPEG_TOKEN_KIND_FOLLOWINGSIBLING,
    XPEG_TOKEN_KIND_CASE,
    XPEG_TOKEN_KIND_ANCESTORORSELF,
    XPEG_TOKEN_KIND_OPEN_BRACKET,
    XPEG_TOKEN_KIND_SOME,
    XPEG_TOKEN_KIND_PROCESSINGINSTRUCTION,
    XPEG_TOKEN_KIND_AND,
    XPEG_TOKEN_KIND_IS,
    XPEG_TOKEN_KIND_DOUBLE_SLASH,
    XPEG_TOKEN_KIND_CLOSE_BRACKET,
    XPEG_TOKEN_KIND_ELSE,
    XPEG_TOKEN_KIND_ANCESTOR,
    XPEG_TOKEN_KIND_VALEQ,
    XPEG_TOKEN_KIND_FOR,
    XPEG_TOKEN_KIND_BY,
    XPEG_TOKEN_KIND_DIVIDE,
    XPEG_TOKEN_KIND_DOUBLE_PIPE,
    XPEG_TOKEN_KIND_BANG,
    XPEG_TOKEN_KIND_ASCENDING,
    XPEG_TOKEN_KIND_ORDER,
    XPEG_TOKEN_KIND_WHERE,
    XPEG_TOKEN_KIND_DOLLAR,
    XPEG_TOKEN_KIND_SELF,
    XPEG_TOKEN_KIND_PRECEDINGSIBLING,
    XPEG_TOKEN_KIND_DESCENDING,
    XPEG_TOKEN_KIND_FALSE,
    XPEG_TOKEN_KIND_OPEN_PAREN,
    XPEG_TOKEN_KIND_VALGE,
    XPEG_TOKEN_KIND_NOT_EQUAL,
    XPEG_TOKEN_KIND_CLOSE_PAREN,
    XPEG_TOKEN_KIND_MODULO,
    XPEG_TOKEN_KIND_EVERY,
    XPEG_TOKEN_KIND_TIMES,
    XPEG_TOKEN_KIND_VALLT,
    XPEG_TOKEN_KIND_AT,
    XPEG_TOKEN_KIND_PLUS,
    XPEG_TOKEN_KIND_DECLARE,
    XPEG_TOKEN_KIND_PRECEDING,
    XPEG_TOKEN_KIND_COMMA,
    XPEG_TOKEN_KIND_MINUS,
    XPEG_TOKEN_KIND_DOT_DOT,
    XPEG_TOKEN_KIND_DOT,
    XPEG_TOKEN_KIND_VALNE,
    XPEG_TOKEN_KIND_FORWARD_SLASH,
    XPEG_TOKEN_KIND_EXCEPT,
    XPEG_TOKEN_KIND_GE_SYM,
    XPEG_TOKEN_KIND_DOUBLE_COLON,
    XPEG_TOKEN_KIND_UNION,
    XPEG_TOKEN_KIND_SHIFT_RIGHT,
    XPEG_TOKEN_KIND_DEFAULT,
    XPEG_TOKEN_KIND_OR,
    XPEG_TOKEN_KIND_CHILD,
    XPEG_TOKEN_KIND_PARENT,
    XPEG_TOKEN_KIND_DESCENDANT,
    XPEG_TOKEN_KIND_ASSIGN,
    XPEG_TOKEN_KIND_DESCENDANTORSELF,
    XPEG_TOKEN_KIND_COLON,
    XPEG_TOKEN_KIND_SEMI_COLON,
    XPEG_TOKEN_KIND_OPEN_CURLY,
    XPEG_TOKEN_KIND_THEN,
    XPEG_TOKEN_KIND_LT_SYM,
    XPEG_TOKEN_KIND_PIPE,
    XPEG_TOKEN_KIND_EQUALS,
};

@interface XPEGParser : PKParser

@end

